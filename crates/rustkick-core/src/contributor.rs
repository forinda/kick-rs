...
pub trait ContextContributor: Send + Sync + 'static {
    ...
    async fn resolve(
        ...
        // `RequestContext` lives in `rustkick-http`.
...
/// Transport-agnostic view of a request — `rustkick-http` implements this
/// for `RequestContext`. Enables `rustkick-core` to stay free of axum
