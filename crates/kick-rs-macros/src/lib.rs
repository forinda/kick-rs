...
/// Same as [`resolve_kick_path`] but for the HTTP crate. Resolves to
/// the umbrella's HTTP re-export when present, falling back to a
/// direct `kick-rs-http` dependency. Used by the `paths!` macro to
/// emit references to `openapi::record_path`.
fn resolve_http_path() -> TokenStream2 {
    if let Ok(found) = crate_name("kick-rs-http") {
        return path_for(found);
    }
    if let Ok(found) = crate_name("kick-rs") {
        // Through the umbrella; we re-export `kick_rs_http` as
        // `kick_rs::__http` for macro use.
        let base = path_for(found);
        return quote! { #base::__http };
    }
    quote! { ::kick_rs_http }
}
...
// `#[delete(...)]` — collocate HTTP method + path with the handler.
//
...
// — the macros are sugar, not a replacement).
//
// ```ignore
// #[get("/users/:id")]
// async fn show(svc: Inject<UserService>, Path(id): Path<Uuid>) -> Json<User> { … }
//
// define_module("users")
//     .handler(show_route)
//     .build()
...
/// `#[get("/path")]` — collocate a GET route with its handler.
#[proc_macro_attribute]
pub fn get(attr: TokenStream, item: TokenStream) -> TokenStream {
    route_attr(attr, item, "get")
}

/// `#[post("/path")]` — collocate a POST route with its handler.
#[proc_macro_attribute]
pub fn post(attr: TokenStream, item: TokenStream) -> TokenStream {
    route_attr(attr, item, "post")
}

/// `#[put("/path")]` — collocate a PUT route with its handler.
#[proc_macro_attribute]
pub fn put(attr: TokenStream, item: TokenStream) -> TokenStream {
    route_attr(attr, item, "put")
}

/// `#[patch("/path")]` — collocate a PATCH route with its handler.
#[proc_macro_attribute]
pub fn patch(attr: TokenStream, item: TokenStream) -> TokenStream {
    route_attr(attr, item, "patch")
}

/// `#[delete("/path")]` — collocate a DELETE route with its handler.
#[proc_macro_attribute]
pub fn delete(attr: TokenStream, item: TokenStream) -> TokenStream {
    route_attr(attr, item, "delete")
}

/// `#[handler]` — pass-through marker.
///
...
/// `paths!(handler1, handler2)` — produce an array of utoipa path
/// recorders for `HttpModuleBuilder::openapi_paths`.
///
/// For each comma-separated path, the macro expands to a reference to
/// `kick_rs_http::openapi::record_path::<__path_<last_seg>>` — the
/// `__path_<fn_name>` type generated by `#[utoipa::path(...)]` next to
/// the handler. Qualified paths work too (`paths!(api::v1::list, get)`):
/// the `__path_` prefix is applied to the last segment, the parent
/// path is preserved.
///
/// Requires `kick-rs-http`'s `openapi` feature.
///
/// ```ignore
/// use kick_rs::paths;
///
...
pub fn paths(input: TokenStream) -> TokenStream {
    ...
    if items.is_empty() {
        return syn::Error::new(
            ...
            "paths! requires at least one handler name",
        )
        .to_compile_error()
        .into();
    }

    let http = resolve_http_path();
    let recorders = items.iter().map(|p| {
        // Build `<parent::__path_<last>>` while preserving the parent
        // segments, so `paths!(api::v1::list)` becomes
        // `api::v1::__path_list` (which is where utoipa-gen places the
        // generated type).
        let mut prefixed = p.clone();
        if let Some(last) = prefixed.segments.last_mut() {
            let new_ident = format_ident!("__path_{}", last.ident);
            last.ident = new_ident;
            last.arguments = syn::PathArguments::None;
        }
        quote! { #http::openapi::record_path::<#prefixed> as #http::module::OpenApiPathRecorder }
    });
...
fn route_attr(attr: TokenStream, item: TokenStream, method: &str) -> TokenStream {
    // Parse the path string. `#[get("/users/:id")]` → `LitStr("/users/:id")`.
