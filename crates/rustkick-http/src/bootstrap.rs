...
impl Bootstrap {
    ...
    pub async fn listen(self, addr: &str) -> KickResult<()> {
        ...
        tracing::info!(target: "rustkick", addr = %local, "listening");
        ...
        for a in &state.adapters {
            if let Err(e) = a.after_start(&ctx).await {
                tracing::warn!(target: "rustkick", adapter = %a.name(), error = %e, "after_start error");
            }
        }

        let shutdown_signal = async {
            let _ = tokio::signal::ctrl_c().await;
            tracing::info!(target: "rustkick", "shutdown signal received");
        };
    ...
    let futures = adapters.iter().map(|a| {
        ...
        async move {
            ...
            match tokio::time::timeout(per_timeout, a.shutdown()).await {
                Ok(Ok(())) => tracing::info!(target: "rustkick", adapter = %name, "shut down cleanly"),
                Ok(Err(e)) => tracing::warn!(target: "rustkick", adapter = %name, error = %e, "shutdown failed"),
                Err(_) => tracing::warn!(target: "rustkick", adapter = %name, "shutdown timed out"),
            }
        }
    });
